...
export default async function handler(req, res) {
  ...
  const publicCategoryPage = resolvePublicCategoryPage(pathname);
  if (publicCategoryPage) {
    send(res, 200, renderCategoryPage(publicCategoryPage), { "Content-Type": "text/html; charset=utf-8" });
    return;
  }
  ...
  if (pathname === "/api/search") {
    ...
    const limit = Math.max(1, Math.min(Number(url.searchParams.get("limit") || "24"), maxLimit));
