...
test("home-data expoe apenas metricas publicas e menu seguro", () => {
  ...
  assert.equal(data.totalPublishedProducts, 2236);
  assert.equal(data.publicCatalogSummary.publishedProducts, 2236);
  ...
  assert.ok(data.topSources.some((entry) => entry.source === "Amazon" && entry.count === 350));
  assert.ok(data.topSources.some((entry) => entry.source === "Mercado Livre" && entry.count === 214));
