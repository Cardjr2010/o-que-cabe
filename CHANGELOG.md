...
- Delayed creation of feed/provider singletons inside `api/web.js` so the function stops booting heavyweight providers at module load.
...
- Separated the API routes and the home rewrite again so `/api/*` keeps using `api/web.js` and `/` falls back to the handler without changing the API wiring.
